<?php

include 'config.php';
$servername = $config['DB_HOST'];
$username = $config['DB_USERNAME'];
$password = $config['DB_PASSWORD'];
$dbname = $config['DB_DATABASE'];   

// Create connection
$conn = new mysqli($servername, $username, $password, $dbname);
// Check connection
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}
$output_array = array(); 
// if (empty($_POST['userName']) || empty($POST['userPassword']) || empty($_POST['userEmail'])){
//     echo "Khali Requesto Nai Padvani... Honnnn";
// }else{
        
    $get_all_results_sql = "select clg.college_name,clg.college_city,b.branch_name,s.std_id,s.std_name from students s,choice c ,branch b,college clg where choice_id in (select choice_id from allocation) and c.branch_id = b.branch_id and c.college_id = clg.college_id and c.std_id = s.std_id";


        if ($res = $conn->query($get_all_results_sql)) {
            if($res->num_rows > 0){

                $row_id = 0;

                while($row = $res->fetch_assoc()){

                    
                    $rcollege_city = $row['college_city'];
                    $rcollege_name = $row['college_name'];
                    $rstd_id = $row['std_id'];
                    $rstd_name = $row['std_name'];
                    $rbranch_name = $row['branch_name'];
    
                    $output_array["success"] = "true";
                   
                    $clip_array = array();
    
                    $clip_array["branch_name"] = $rbranch_name;
                    $clip_array["college_city"] = $rcollege_city;
                    $clip_array["college_name"] = $rcollege_name;
                    $clip_array["std_id"] = $rstd_id;
                    $clip_array["std_name"] = $rstd_name;

                    array_push($output_array,$clip_array);
                    
                    $row_id =  $row_id + 1;
                }

            }else{
                $output_array["success"] = "false";
            }
        } else {
            $output_array["error"]  = "Error: " . $sql . "Err" . $conn->error;
        }
// }

echo json_encode($output_array,JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE | JSON_PARTIAL_OUTPUT_ON_ERROR);
$conn->close();
?>
